...
function generateInvoiceTable(doc, invoiceItems, counts) {
    let i, invoiceTableTop = 450;
    let position;
    doc
        .fillColor('grey')
        .rect(0, 410, 1080, 40)
    ...
    for (i = 0; i < invoiceItems.length; i++) {
        ...
        generateTableRow(doc, position, item.description, counts[item.description], amount, item.total);
        doc
