...
import { CompanyMemberInvitation as CompanyMemberInvitationDb, User as UserDb, Company as CompanyDb, CompanyMember as CompanyMemberDb, } from '../config/dbConfig.js';
...
export const CompanyMemberInvitationResolver = {
    Query: {
        async getInvitationByUserId(_, _args, context) {
    ...
    Mutation: {
        async createCompanyMemberInvitation(_, args, context) {
            try {
                ...
                //Check if user exist
                if (!user) {
                    throwCustomError('There is no user with this email', ErrorType.NOT_FOUND);
                }
                ...
                if (invitation) {
                    sendMail(user.email, 'Invitation', `${user.username}, you got invited to the company`);
                }
                return invitation;
            }
            catch (err) {
        ...
        async softDeleteInvitationAfterResponse(_, args, context) {
            try {
    ...
    CompanyMemberInvitation: {
        async userId(invitation) {
            return UserDb.findByPk(invitation.userId);
        },
        async companyId(invitation) {
