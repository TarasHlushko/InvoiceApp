...
const syncDbTables = async () => {
  await User.sync({ alter: true }).catch((err) =>
    console.log('Could not create model User', err)
  );
  await Company.sync({ alter: true }).catch((err) =>
    console.log('Could not create model Company', err)
  );
  await Client.sync({ alter: true }).catch((err) =>
    console.log('Could not create model Client', err)
  );
  await Invoice.sync({ alter: true }).catch((err) =>
    console.log('Could not create model Invoice', err)
  );
  await InvoiceItems.sync({ alter: true }).catch((err) =>
    console.log('Could not create model InvoiceItems', err)
  );
  await CompanyMember.sync({ alter: true }).catch((err) =>
    console.log('Could not create model CompanyMember', err)
  );
  await CompanyMemberInvitation.sync({ alter: false }).catch((err) =>
    console.log('Could not create model CompanyMemberInvitation', err)
  );
};
