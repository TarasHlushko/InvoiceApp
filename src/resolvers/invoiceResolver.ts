...
import {
  ...
  InvoiceItems as InvoiceItemsDb,
  User as UserDb,
} from '../config/dbConfig.js';
import { IContext } from '../util/ContextInterface';
import { IInvoice } from '../util/invoiceInterfaces/invoiceInterface';
import { ICreateInvoiceInput } from '../util/invoiceInterfaces/createInvoiceInputInterface';
import { IUpdateInvoiceInput } from '../util/invoiceInterfaces/updateInvoiceInputInterface';
...
export const invoiceResolver = {
  Query: {
    async getInvoiceById(_, args: IInvoice, context: IContext) {
    ...
    async listInvoicesByCompany(
      _,
      args: IFindEntitiesInput,
      context: IContext
    ) {
      // Check for authorization
      const companyMember = await CompanyMemberDb.findByPk(
  ...
  Invoice: {
    async invoiceItems(parent: IInvoice) {
  ...
  Mutation: {
    async createInvoice(_, args: ICreateInvoiceInput, context: IContext) {
      try {
        const companyMember = await CompanyMemberDb.findByPk(
          context.tokenPayload.id
        );
    ...
    async updateInvoice(_, args: IUpdateInvoiceInput, context: IContext) {
      try {
        const companyMember = await CompanyMemberDb.findByPk(
          context.tokenPayload.id
        );
        ...
        for (const companyMember of companyMembers) {
          ...
          await sendMail(
            ...
            `One of the invoice's status has been changed`
          );
        }
    ...
    async softDeleteInvoice(_, args: IInvoice, context: IContext) {
      const companyMember = await CompanyMemberDb.findByPk(
        context.tokenPayload.id
      );
      const invoice = await InvoiceDb.findByPk(args.id);
    ...
    async generatePDFByInvoiceId(_, args, context: IContext) {
      const companyMember = await CompanyMemberDb.findByPk(
        context.tokenPayload.id
      );
      const invoice = await InvoiceDb.findByPk(args.id);
