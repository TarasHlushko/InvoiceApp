...
import { ICreateUserInput } from '../util/userResolverInterfaces/CreateUserInputInterface';
import { IUpdateUserInput } from '../util/userResolverInterfaces/UpdateUserInputInterface';
import { IUser } from '../util/userResolverInterfaces/UserInterface';
...
const userResolver = {
  Query: {
    async listUsersByCompany(_, args: IFindEntitiesInput, context: IContext) {
    ...
    async getUserById(_, args: IUser, context: IContext) {
      ...
      const loggedCompanyMember = await CompanyMemberDb.findByPk(
        context.tokenPayload.id
      );

      if (
        searchedCompanyMember &&
        loggedCompanyMember &&
  ...
  Mutation: {
    async createUser(_, args: ICreateUserInput) {
      try {
    ...
    async updateUser(_, args: IUpdateUserInput, context: IContext) {
      try {
        const isUserExist = await UserDB.findOne({
          where: {
            id: args.id,
    ...
    async softDeleteUser(_, args: IUser, context: IContext) {
      ...
      const userToDeleteCompanyMember = await CompanyMemberDb.findByPk(
        userToDelete.id
      );
